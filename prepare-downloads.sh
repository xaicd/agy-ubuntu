...
# ---------- 1. mihomo ----------
echo "[1/3] mihomo ${MIHOMO_VERSION}"
...
# ---------- 3. apt debs ----------
echo "[3/3] apt 依赖包(ca-certificates/curl/git/iproute2/iptables/nano/tzdata/xdg-utils)"
DEB_COUNT="$(ls "$DL_DIR/debs"/*.deb 2>/dev/null | wc -l | tr -d ' ')"
if [ "${DEB_COUNT:-0}" -ge 50 ]; then
    echo "    已有 $DEB_COUNT 个 .deb,跳过"
else
...
apt-get install --download-only -y --no-install-recommends -o Dir::State::status=/dev/null -o Dir::State::extended_states=/dev/null ca-certificates curl git iproute2 iptables nano tzdata xdg-utils >/dev/null
